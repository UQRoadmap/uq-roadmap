...
from degree.params import ProgramRef, CourseRef, EquivalenceGroup, create_course_or_equivalence_from_dict
...
class SR3(SR):
    ...
    def validate(
        ...
        return ValidateResult(Status.OK, 100, "", [], self.part)
...
class SR7(SR):
    ...
    def validate(
        ...
        return ValidateResult(
            Status.OK,
            100.0,
            f"Complete exactly {self.n} {self.plan_types} from following {option_codes}",
            option_codes,
            self.part,
        )
...
class SR8(SR):
    ...
    def validate(
        ...
        return ValidateResult(
            Status.OK,
            100.0,
            f"Complete {self.n} to {self.m} {self.plan_types} from following {option_codes}",
            option_codes,
            self.part,
        )
...
def create_sr_from_dict(data: dict) -> SR:
    ...
    if sr_type in ["SR1", "SR2", "SR3", "SR4", "SR5"] and "options" in data:
        ...
        for option_data in options_data:
            converted_options.append(create_course_or_equivalence_from_dict(option_data))
        ...
        if sr_type in type_map:
            ...
            if sr_type in ["SR2", "SR4"]:
                ...
                return sr_class(
                    part=data_copy.get("part", ""), n=data_copy["n"], options=data_copy["options"], type=sr_type
                )
